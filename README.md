...
The parameter grid used in the implementation was:
...
&=
2^{1-\zeta_m}
\sum*{\substack{j<k \ R_{ij},R_{ik}\le R_{\mathrm{cut}}}}
...
The cutoff was applied only to the central–neighbor distances $R_{ij}$ and $R_{ik}$, while $R_{jk}$ appears only inside the exponential.
...
angular functions per **unordered element pair**. With three elements, the unordered pairs are:
...
The implemented model follows the **second-generation HDNNP architecture** shown in Figure 1. Each atomic network maps the local ACSF descriptor of an atom to a scalar atomic energy contribution, and the total molecular energy is obtained by summation:

[
\hat{E} = \sum_{i=1}^{N} \hat{E}*i
= \sum*{\mu}\sum_{i \in \mu} \mathrm{NN}_{\mu}(\mathbf{G}_i^\mu).
]

For the malonaldehyde molecule, **three element-specific atomic neural networks** were constructed corresponding to carbon (C), hydrogen (H), and oxygen (O). Each atomic network used:

* Input dimension: $D = 216$
* Two fully connected hidden layers (64 neurons each)
* $\tanh$ activation
* Linear output layer producing a scalar atomic energy

### Regularization

To improve generalization and reduce overfitting:

* **L2 weight regularization** ($\lambda = 10^{-6}$) was applied to all dense layers
* **Dropout** with rate $0.05$ was applied after each hidden layer [3]

---

### Normalization and Optimization

Feature and energy standardization were applied using training-set statistics:
...
The model was trained using **mean-squared error (MSE)** loss and the **Adam optimizer** [4] with learning rate $10^{-4}$. Training details:
...
* Callbacks:
  ...
  * Reduce-on-plateau (`factor = 0.25`, `min_lr = 10^{-6}`)
...
Performance was evaluated on a held-out test set using:
...
Final test performance (native energy units):
